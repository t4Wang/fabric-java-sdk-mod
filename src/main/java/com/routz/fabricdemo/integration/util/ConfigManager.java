...
//            System.getenv(ORG_HYPERLEDGER_FABRIC_SDK_TEST_FABRIC_HOST) == null ? "localhost" : System.getenv(ORG_HYPERLEDGER_FABRIC_SDK_TEST_FABRIC_HOST);
            System.getenv(ORG_HYPERLEDGER_FABRIC_SDK_TEST_FABRIC_HOST) == null ? "192.168.1.66" : System.getenv(ORG_HYPERLEDGER_FABRIC_SDK_TEST_FABRIC_HOST);
    ...
    private ConfigManager() {
        ...
        try {
            ...
            loadFile = new File(System.getProperty(ORG_HYPERLEDGER_FABRIC_SDK_CONFIGURATION, DEFAULT_CONFIG))
                    .getAbsoluteFile();
            logger.debug(String.format("加载配置 %s 当前为: %b", loadFile.toString(),
                    loadFile.exists()));
...
//                    DEFAULT_CONFIG));
            ...
            for (Map.Entry<Object, Object> x : sdkProperties.entrySet()) {
                ...
                if (key.startsWith(INTEGRATIONTESTS_ORG)) {
                    ...
                    if (match.matches() && match.groupCount() == 1) {
                        ...
                        fabricOrgs.put(orgName, new FabricOrg(orgName, val.trim()));
            ...
            try {
                for (Map.Entry<String, FabricOrg> org : fabricOrgs.entrySet()) {
                    ...
                    // ps : peer0.org1.example.com@grpc://192.168.1.110:7051
                    String[] ps = peerNames.split("[ \t]*,[ \t]*");
                    for (String peer : ps) {
                        String[] nl = peer.split("[ \t]*@[ \t]*");
                        // peer location: peer0.org1.example.com
                        // grpcTLSify: grpc://192.168.1.110:7051
                    ...
                    // orderer.example.com@grpc://192.168.1.110:7050
                    String ordererNames = sdkProperties.getProperty(INTEGRATIONTESTS_ORG + orgName + ".orderer_locations");
                    ps = ordererNames.split("[ \t]*,[ \t]*");
                    for (String peer : ps) {
                        String[] nl = peer.split("[ \t]*@[ \t]*");
                        fabricOrg.addOrdererLocation(nl[0], grpcTLSify(nl[1]));
                    }

                    // http://192.168.1.110:7054 http://192.168.1.110:8054
                    fabricOrg.setCALocation(httpTLSify(sdkProperties.getProperty((INTEGRATIONTESTS_ORG + org.getKey() + ".ca_location"))));
                    fabricOrg.setCAName(sdkProperties.getProperty((INTEGRATIONTESTS_ORG + org.getKey() + ".caName")));
    ...
    /**
     * @param location grpc://192.168.1.110:7051
     * @return
     */
    ...
    public Collection<FabricOrg> getIntegrationTestsFabricOrgs() {
        return Collections.unmodifiableCollection(fabricOrgs.values());
    }

    public FabricOrg getIntegrationTestsFabricOrg(String name) {
        return fabricOrgs.get(name);
    ...
    public static void main(String[] ars) throws MalformedURLException, InvalidArgumentException {

        final ConfigManager config = getConfig();
