...
import static com.routz.fabricdemo.integration.util.Print.assertFalse;
import static com.routz.fabricdemo.integration.util.Print.assertNotNull;
import static com.routz.fabricdemo.integration.util.Print.assertNull;
...
import static com.routz.fabricdemo.integration.util.Print.printableString;
...
public class ChannelTest {
    private static final ConfigManager testConfig = ConfigManager.getConfig();

    FabricStore fabricStore = FabricStore.setupFabricStore();
    ...
    private final ConfigUtils configHelper = new ConfigUtils();
    String testTxID = null;  // save the CC invoke TxID and use in queries
    private Collection<FabricOrg> testFabricOrgs = testConfig.getIntegrationTestsFabricOrgs();
    ...
    public void constructChannel(String channelName, FabricOrg fabricOrg) throws Exception {
        checkConfig();
...
//        FabricOrg fabricOrg = testConfig.getIntegrationTestsFabricOrg("peerOrg1");
        ...
        // Only peer Admin org
        FabricUser peerAdmin = setupPeerAdmin(fabricOrg);
        fabricOrg.setPeerAdmin(peerAdmin);      // A special user that can create channels, join peers and install chaincode
        client.setUserContext(peerAdmin);

        Collection<Orderer> orderers = setupOrderers(client, fabricOrg);
        //Just pick the first orderer in the list to create the channel.
        // 一个orderer节点 7050
        Orderer anOrderer = orderers.iterator().next();
        orderers.remove(anOrderer);

        Channel newChannel = createChannel(client, channelName, peerAdmin, anOrderer);

        Collection<Peer> peers = setupPeers(client, fabricOrg);
...
//        Channel fooChannel = client.deSerializeChannel(serializedChannelBytes).initialize();
    ...
    public void installChaincode() throws Exception {
        try {
            ...
            HFClient client = HFClient.createNewInstance();
            client.setCryptoSuite(CryptoSuite.Factory.getCryptoSuite());
            FabricUser sampleUser = setupPeerAdmin(fabricOrg);
            fabricOrg.setPeerAdmin(sampleUser);
            client.setUserContext(fabricOrg.getPeerAdmin());
            Channel channel = client.newChannel(channelName);
            Collection<Orderer> orderersSetup = setupOrderers(client, fabricOrg);
             ...
             */
            //   client.setUserContext(fabricOrg.getUser(TEST_ADMIN_NAME));
    ...
    public void sendTransaction() throws Exception {
        ...
        // HFClient client, Channel channel, boolean installChaincode, FabricOrg fabricOrg, int delta
        boolean installChaincode = true;
        ...
        try {
            ...
            FabricUser user = fabricStore.getMember(userName, fabricOrg.getName());
            ...
            client.setUserContext(fabricOrg.getUser("user2"));
    ...
    public void queryTransaction() {
        try {
            ...
            File chaincodeMetaInfLocation = new File("src/test/fixture/meta-infs/end2endit");
            File chaincodeEndorsementPolicyYamlFile = new File(TEST_FIXTURES_PATH + "/sdkintegration/chaincodeendorsementpolicy.yaml");
            FabricOrg sampleOrg = testConfig.getIntegrationTestsFabricOrg("peerOrg1");
            String channelName = FOO_CHANNEL_NAME;
            String chaincodeInitFunctionName = "init";
            int delta = 0;
            String[] chaincodeInitFunctionArgs = {"a", "500", "b", "" + (200 + delta)};
            ...
            HFClient client = HFClient.createNewInstance();
            client.setCryptoSuite(CryptoSuite.Factory.getCryptoSuite());
            FabricUser sampleUser = setupPeerAdmin(sampleOrg);
            ...
            // HFClient client, Channel channel, boolean installChaincode, FabricOrg sampleOrg, int delta
            boolean installChaincode = true;
            ...
            String userName = "user2";
            FabricUser user = fabricStore.getMember(userName, sampleOrg.getName());
            user.setEnrollmentSecret("ulRAxurceJZg");
            // 登记用户
            if (!user.isEnrolled()) {
                user.setEnrollment(ca.enroll(user.getName(), user.getEnrollmentSecret()));
    ...
    private String getChaincodeEventListenerHandler(Channel channel, Vector<ChaincodeEventCapture> chaincodeEvents) throws org.hyperledger.fabric.sdk.exception.InvalidArgumentException {
    ...
    private void addPeerToChannel(Channel channel, Collection<Peer> peers) throws org.hyperledger.fabric.sdk.exception.InvalidArgumentException {
        for (Peer peer : peers ) {
            // 给通道加入节点
    ...
    private Collection<Peer> setupPeers(HFClient client, FabricOrg sampleOrg) throws InvalidArgumentException {
    ...
    private Collection<Orderer> setupOrderers(HFClient client, FabricOrg sampleOrg) throws InvalidArgumentException {
    ...
    private FabricUser setupPeerAdmin(FabricOrg sampleOrg) throws IOException, NoSuchAlgorithmException, NoSuchProviderException, InvalidKeySpecException, InvalidArgumentException {
        // peerAdmin
    ...
    private Channel createChannel(HFClient client, String name, FabricUser peerAdmin, Orderer orderer) throws IOException, InvalidArgumentException, TransactionException {
    ...
    public void runFabricTest() throws Exception {
        final FabricStore fabricStore = null;
        ...
        fabricStore.saveChannel(fooChannel);
        ...
        sampleOrg = testConfig.getIntegrationTestsFabricOrg("peerOrg2");
        Channel barChannel = constructChannel(BAR_CHANNEL_NAME, client, sampleOrg);
        assertTrue(barChannel.isInitialized());
        /**
         * fabricStore.saveChannel uses {@link Channel#serializeChannel()}
         */
        fabricStore.saveChannel(barChannel);
    ...
    void runChannel(HFClient client, Channel channel, boolean installChaincode, FabricOrg sampleOrg, int delta) {
...
//        testRemovingAddingPeersOrderers(client, channel);
        ...
        try {
            ...
            if (installChaincode) {
                ...
                //   for (FabricOrg org : testSampleOrgs) {
    ...
    Channel constructChannel(String name, HFClient client, FabricOrg sampleOrg) throws Exception {
        ////////////////////////////
...
//        boolean doPeerEventing = !testConfig.isRunningAgainstFabric10() && FOO_CHANNEL_NAME.equals(name);
        // Only peer Admin org
        // eroll的用户
        FabricUser peerAdmin = sampleOrg.getPeerAdmin();
        client.setUserContext(peerAdmin);
